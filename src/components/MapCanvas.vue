...
export default {
  ...
  mounted() {
    ...
    // Funções auxiliares para lidar com o carregamento de imagens
    const setupAndDraw = () => {
      if (this.image !== null) {
        ...
        // Verifica se a imagem do grid já está completa (no cache)
        const loadGridImage = () => {
          this.setZoom();
          this.requestDraw();
        ...
        if (this.gridImage.complete) {
          loadGridImage();
        } else {
          this.gridImage.onload = loadGridImage;
        }
      }
    };
    ...
    // Verifica se a imagem principal já está completa (no cache)
    if (this.image.complete) {
      setupAndDraw();
    } else {
      this.image.onload = setupAndDraw;
    }
